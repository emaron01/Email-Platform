/**
 * Node-safe usage/research policy resolver (no server-only).
 * Next.js entry: `@/lib/usage/policy` re-exports behind server-only.
 */
...
async function ensureResearchPolicyOnly(organizationId: string): Promise<void> {
...
export async function ensureUsageAndResearchPolicies(
  ...
  await ensureResearchPolicyOnly(organizationId);
}
...
export async function getEffectiveUsagePolicy(input: {
  ...
  await ensureUsageAndResearchPolicies(input.organizationId);
